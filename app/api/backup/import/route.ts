...
export async function POST(request: Request) {
  try {
    ...
    const company = body.company as { id?: unknown } | undefined;
    const companyId = requireValidCompanyId(body.companyId ?? company?.id);
    ...
    await db.runTransaction(async (tx) => {
      tx.set(
        companyDocRef,
        {
          ...
          // createdAt is left to server timestamp; merge safe
        },
        { merge: true }
      );
    });
    ...
    for (const m of members) {
      ...
      // Existing members keep their current role; the importing admin keeps their role.
      if (!beforeSnap.exists && email !== permission.email) continue;
      const role = email === permission.email
        ? (normalizeRole(memberLike.role) || permission.role)
        : (normalizeRole(before?.role) || "viewer");
