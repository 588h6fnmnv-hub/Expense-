...
import { enforceRateLimit, parseJsonObject, requireValidCompanyId } from "@/lib/security";
import {
  AccountBalances,
  CardItem,
  CompanyProfile,
...
const normalizePersonAccounts = (value: unknown): PersonAccount[] => {
  ...
  value.forEach((account) => {
    ...
    const nextAccount: PersonAccount = {
      ...
      // referralCode intentionally omitted — invite codes must never be stored
      // in the wallet payload. They are written and immediately invalidated by
      // the invite flow; persisting them here would expose active codes to any
      // reader of the company-scoped wallet collection.
      referralCode: "",
    };
...
export async function GET(request: Request) {
  ...
  await enforceRateLimit({ request, key: "wallet:read", limit: 120, windowMs: 60_000, userEmail: context.email });
...
export async function POST(request: Request) {
  ...
  await enforceRateLimit({ request, key: "wallet:write", limit: 60, windowMs: 60_000, userEmail: context.email });
