...
export default function Home() {
  ...
  const startEmployeeLogin = () => {
    ...
    setWalletReady(true);
    setStorageStatus("local");
    setTab(invite.role === "worker" ? "Workers" : "Sites");
  };
  ...
  if (!savedUser) {
    return (
      <main className="min-h-dvh overflow-y-auto bg-black px-6 py-8 text-white">
        <form
          onSubmit={(e) => {
            e.preventDefault();
          }}
          className="mx-auto w-full max-w-sm rounded-[34px] border border-white/15 bg-white/10 p-6 shadow-2xl backdrop-blur-2xl"
        >
          <div className="mx-auto mb-6 h-20 w-20 rounded-[24px] bg-white text-center text-5xl leading-[80px] shadow-xl">
            💳
          </div>
          ...
          <p className="mt-2 text-center text-white/60">
            Use Google to open your money dashboard
          </p>

          <button
            type="button"
            className="mt-8 flex w-full items-center justify-center gap-3 rounded-3xl bg-white p-4 font-extrabold text-black transition active:scale-[0.98]"
            onClick={() => {
              if (!acceptedTerms) {
          ...
          >
            <span className="grid h-6 w-6 place-items-center rounded-full bg-black text-sm text-white">
              G
            </span>
          ...
          <button
            type="button"
            className="mt-3 w-full rounded-3xl bg-emerald-500 p-4 font-extrabold text-black transition active:scale-[0.98]"
            onClick={startDemoCompany}
          >
            Open Demo Company
          </button>

          <div className="mt-3 rounded-3xl border border-white/15 bg-white/5 p-3">
            <input
              ...
              className="w-full rounded-2xl border border-white/10 bg-white px-4 py-3 text-sm font-bold text-black outline-none placeholder:text-neutral-400"
            />
            <button
              type="button"
              onClick={startEmployeeLogin}
              className="mt-2 w-full rounded-2xl bg-white/10 px-4 py-3 text-sm font-extrabold text-white transition active:scale-[0.98]"
            >
          ...
          <a
            href="/demo"
            className="mt-3 block rounded-3xl border border-white/15 bg-white/5 p-4 text-center font-extrabold text-white transition active:scale-[0.98]"
          >
            View public demo page
          </a>

          <label className="mt-4 flex gap-3 rounded-3xl border border-white/10 bg-white/5 p-4 text-sm leading-5 text-white/70">
            <input
              type="checkbox"
            ...
            />
            <span>
              I read and agree that this app stores wallet entries, balances,
              cards, settings, and reports. If a backend/admin panel is added,
              the admin may access user app data for support, security, and
              account management. Email automation will require explicit Gmail
              or Outlook permission and should only read transaction alerts.
            </span>
          </label>

          {loginError && (
            <p className="mt-3 rounded-2xl bg-red-500/15 p-3 text-sm font-bold text-red-100">
              {loginError}
            </p>
          )}
        </form>
    ...
    return (
      <main className="min-h-dvh overflow-y-auto bg-black px-6 py-8 text-white">
        ...
        >
          ...
          <input
            className="mt-8 w-full rounded-3xl border border-white/15 bg-white p-4 text-black outline-none placeholder:text-neutral-400 focus:ring-2 focus:ring-white/40"
          ...
          <button
            type="submit"
            className="mt-4 w-full rounded-3xl bg-white p-4 font-extrabold text-black transition active:scale-[0.98]"
          >
            Continue
          </button>
          ...
          <button
            ...
            className="mt-3 w-full rounded-3xl border border-white/15 bg-white/10 p-4 font-extrabold text-white transition active:scale-[0.98]"
          >
  ...
  if (showWelcome) {
    return (
      <main className="relative flex min-h-screen flex-col items-center justify-center bg-black px-6 text-white">
        <p className="text-3xl font-black text-white/55">{APP_NAME}</p>
        ...
        <button
          ...
          className="absolute bottom-12 animate-bounce text-center text-sm text-white/70"
        >
          ⌃
          <br />
  ...
  return (
    <main
      className={`mx-auto min-h-screen max-w-md px-4 pb-28 pt-8 transition-all duration-500 ${theme === "dark"
        ? "wallet-dark bg-black text-white"
