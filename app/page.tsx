...
export default function Home() {
  ...
  useEffect(() => {
    const sessionUserEmail = sessionEmail;
    if (!sessionUserEmail || savedUser === sessionUserEmail) return;

    const userRecord = {
      username: sessionUserEmail,
      email: sessionUserEmail,
      lastLogin: new Date().toISOString(),
    };
    const nextUsers = [
      userRecord,
      ...registeredUsers.filter((user) => user.username !== sessionUserEmail),
    ];

    localStorage.setItem("username", sessionUserEmail);
    localStorage.setItem("authMode", "credentials");
    writeEmployeeSession(null);
    localStorage.setItem("adminUsers", JSON.stringify(nextUsers));
    setRegisteredUsers(nextUsers);
    setEmployeeSession(null);
    setSavedUser(sessionUserEmail);
    setShowWelcome(false);

    // Ensure we don't have stale local data
    const localWallet = readLocalWallet(sessionUserEmail);
