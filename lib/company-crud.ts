...
import { enforceRateLimit, logSecurityEvent, parseJsonObject, requireValidCompanyId } from "@/lib/security";
import {
...
const canReadScopedItem = ({
  ...
  if (permission.role === "worker") {
    ...
    if (collectionName === "materials") return Boolean(projectId) && assignedProjectIds.includes(projectId);
...
export const listCompanyCollection = async ({
  ...
  try {
    ...
    await enforceRateLimit({ request, key: `api:${collectionName}:read`, limit: 120, windowMs: 60_000, userEmail: permission.email });
    ...
    await logSecurityEvent({ type: "api_error", request, companyId, metadata: { collectionName, status: errorToStatus(error), message: errorToMessage(error) } });
...
export const createCompanyCollectionDocument = async ({
  ...
  try {
    ...
    await enforceRateLimit({ request, key: `api:${collectionName}:write`, limit: 60, windowMs: 60_000, userEmail: email });
